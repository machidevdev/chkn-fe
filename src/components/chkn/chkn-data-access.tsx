...
export function useChknProgram() {
  ...
  const programId = useMemo(() => getChknProgramId(cluster.network as Cluster), [cluster])
  ...
  const initialize = useMutation({
    mutationKey: ['chkn', 'initialize', { cluster }],
    mutationFn: (keypair: Keypair) =>
      program.methods.initialize().accounts({ chkn: keypair.publicKey }).signers([keypair]).rpc(),
    onSuccess: (signature) => {
...
export function useChknProgramAccount({ account }: { account: PublicKey }) {
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () => program.methods.decrement().accounts({ chkn: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx)
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () => program.methods.increment().accounts({ chkn: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx)
      return accountQuery.refetch()
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) => program.methods.set(value).accounts({ chkn: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx)
      return accountQuery.refetch()
    },
